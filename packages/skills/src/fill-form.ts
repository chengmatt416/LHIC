...
export async function fillForm(
  ...
  try {
    ...
    if (input.submit) {
      await trace.emit("fill_form_requires_human_approval", {});
      return skillFailure(
        trace,
        "Form fields were filled but not submitted. Execute a separately approved SemanticAction to submit the form.",
        true,
      );
    }
