...
describe("2D game-training core", () => {
  it("uses a two-frame policy and maps Star Trooper's space key to fire", () => {
    ...
    expect(game2dFrameSpec).toMatchObject({ width: 128, height: 128, history: 2 });
    expect(
      encodeGame2dInput(
        {
          timestampMs: 1,
  ...
  it("rejects 3D profiles and rewards score gain while penalizing damage", () => {
    expect(() => validateGame2dProfile(getGameTargetProfile("nemesis"))).toThrow("2D");
    expect(
      game2dReward({ previousScore: 0, score: 100, previousHealth: 100, health: 90, terminal: false }),
    ).toBeCloseTo(0.899);
  });
});
