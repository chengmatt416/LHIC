...
export function validateActionApproval(
  ...
  if (!approval) {
    return {
      ...
      reason: `${policy.reason} No action approval was supplied.`,
    };
  }
  ...
  if (options.requireSignature) {
    if (!options.publicKey) {
      return {
        ...
        requiresConfirmation: true,
        reason:
          "Production high-risk actions require an approval verification public key.",
      };
    }
    if (!approval.signature || !isValidSignature(approval, options.publicKey)) {
      return {
        allowed: false,
        requiresConfirmation: true,
  ...
  return {
    ...
    reason: "High-risk action has a valid, matching human approval.",
