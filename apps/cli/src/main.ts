...
async function main(argumentsList: string[]): Promise<void> {
  ...
  console.error(
    "Usage: lhic preflight | lhic run action <action-file> [approval-file] | lhic bench internal | lhic bench simulate resilience [task-count] [seed] | lhic bench readiness <workarena|webarena> | lhic bench validate-evidence <file> | lhic trace inspect <trace-file>",
  );
  process.exitCode = 1;
}
