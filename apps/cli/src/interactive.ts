...
async function guideUninstallCommand(
  ...
  const guided = [...argumentsList];
  guided[1] ??= await askChoice(prompter, "Uninstall LHIC", ["cli", "desktop"]);
  return guided;
}
