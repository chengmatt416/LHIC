...
export const cliUsage =
  "Usage: lhic [agent [<prompt>] [--omp-policy <pinned|managed|development-latest>] [--omp-version <v>] [--omp-digest <sha256>] | capabilities | install <cli|desktop> | start [memory-database] | demo [--safe] [--viewable] [--endpoint <URL>] | gui [demo|mcp] [--no-open] | shared <enable|login|disable|status|sync|list> [options] | train public-web <wikipedia-search|mdn-search|github-issue-filter|openstreetmap-place-search> --query <safe-public-query> [--database <path>] [--viewable] | train game env <setup|doctor> [--root <path>] [--python <path>] | train game <2d|3d> <setup|lease|record|fit|evaluate|play> <star-trooper|nemesis> [options] | preflight | global doctor | run action <action-file> [approval-file] | run plan <plan-file> [approvals-file] [--var name=value] | bench internal [--output <path>] | bench simulate resilience [task-count] [seed] | bench readiness <workarena|webarena> | bench validate-evidence <file> | mcp config <antigravity|codex|claude-code|vscode> [workspace-root] | trace inspect <trace-file> | trace provenance <trace-file> [--json]]";
