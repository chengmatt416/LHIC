...
<section class="panel" id="demo-panel" role="tabpanel"><h2>Learning Demo</h2><p>Slow Path uses the model one action at a time. After learning, Fast Path compiles one plan then executes locally. A separate visible Chromium window shows the task.</p><form id="demo-form"><div class="grid"><label>Provider<select name="provider"><option value="openai">OpenAI</option><option value="gemini">Gemini</option><option value="claude">Claude</option></select></label><label>Model ID<input name="model" required placeholder="gemini-2.5-flash"></label><label class="wide">Custom endpoint <small>Optional; must match the selected provider protocol.</small><input name="endpoint" type="url" placeholder="https://models.example.com/v1/responses"></label><label class="wide">API key <small>Stored locally in your OS Keychain; never shown in this GUI.</small><input name="apiKey" type="password" required autocomplete="off"></label><label class="wide">Public HTTPS website<input name="websiteUrl" type="url" required placeholder="https://example.com"></label><label class="wide">Slow Path task<textarea name="slowTask" required placeholder="Describe the browser task to demonstrate."></textarea></label></div><div class="actions"><button class="primary" type="submit">Start visible demo</button><span id="demo-state">Ready.</span></div></form><div id="dynamic-input" class="notice hidden"><strong id="dynamic-label"></strong><input id="dynamic-value"><div class="actions"><button class="primary" id="dynamic-send">Continue</button></div></div><div id="demo-log" class="notice" aria-live="polite">The demo browser opens after you start.</div></section>
