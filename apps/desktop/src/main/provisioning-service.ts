...
export class ProvisioningService {
  ...
  private async runOnce(): Promise<StepReport[]> {
    if (process.env.LHIC_SKIP_BACKENDS === "1") {
      return [
        {
          name: "skip",
          message: "provisioning skipped (LHIC_SKIP_BACKENDS=1)",
        },
      ];
    }
    try {
  ...
  private async provisionPeekaboo(report: StepReport[]): Promise<void> {
    ...
    if (await this.commandExists("brew")) {
      ...
      await this.run(
        "npm",
        ["install", "--global", "@steipete/peekaboo"],
        600_000,
      );
    } else {
      report.push({
        name: "peekaboo",
        message:
          "needs Homebrew or npm — install manually (brew install steipete/tap/peekaboo)",
      });
    }
  }
  ...
  private async provisionFlaUI(report: StepReport[]): Promise<void> {
    ...
    if (process.env.LHIC_FLAUI_DLL) {
      try {
        ...
        report.push({
          name: "flaui",
          message: `bridge present (LHIC_FLAUI_DLL=${process.env.LHIC_FLAUI_DLL})`,
        });
        return;
      } catch {
        report.push({
          name: "flaui",
          message: "LHIC_FLAUI_DLL set but not found — building the bridge",
        });
      }
    }
    if (!(await this.commandExists("dotnet"))) {
      if (await this.commandExists("winget")) {
        report.push({
          name: "flaui",
          message: "installing the .NET SDK via winget…",
        });
        await this.run(
          "winget",
          [
        ...
        report.push({
          name: "flaui",
          message:
            "dotnet missing and winget unavailable — install the .NET SDK, then run scripts/build-flaui-helper.ps1",
        });
        return;
      }
    }
    ...
    if (!project) {
      report.push({
        name: "flaui",
        message:
          "bridge sources not bundled — set LHIC_FLAUI_DLL after building scripts/build-flaui-helper.ps1",
      });
      return;
    }
    ...
    const result = await this.run(
      ...
      [
        ...
        "false",
        "-o",
        resolve(this.executionSourceDir!, "flaui", "bin", "win-x64"),
      ],
      600_000,
    );
    report.push(
      result.code === 0
        ? {
            name: "flaui",
            message:
              "bridge built — set LHIC_FLAUI_DLL to execution/flaui/bin/win-x64/lhic-flaui.dll",
          }
        : {
            name: "flaui",
            message: `bridge build failed: ${result.stderr.trim().slice(0, 200)}`,
          },
    );
  }
  ...
  private async provisionOmniParser(report: StepReport[]): Promise<void> {
    if (!(await this.commandExists("python3"))) {
      report.push({
        name: "omniparser",
        message: "skipped — python3 is not available",
      });
      return;
    }
    ...
    if (check.code === 0) {
      report.push({
        name: "omniparser",
        message: "omni_parser_v2 already installed",
      });
      return;
    }
    report.push({
      name: "omniparser",
      message: "installing omni_parser_v2 (pip ladder)…",
    });
    ...
    report.push(
      verified.code === 0
        ? {
            name: "omniparser",
            message: "installed (weights download on first use)",
          }
        : {
            name: "omniparser",
  ...
  private async provisionChromium(report: StepReport[]): Promise<void> {
    if (!(await this.commandExists("npx"))) {
      report.push({
        name: "chromium",
        message: "skipped — npx is not available",
      });
      return;
    }
    report.push({
      name: "chromium",
      message: "installing Playwright Chromium…",
    });
    const result = await this.run(
      "npx",
    ...
    report.push(
      result.code === 0
        ...
        : {
            name: "chromium",
            message: `install failed: ${result.stderr.trim().slice(0, 200)}`,
          },
    );
  }
  ...
  private async windowsVersion(): Promise<
    { major: number; build: number } | undefined
  > {
    const result = await this.run(
      "powershell",
      [
